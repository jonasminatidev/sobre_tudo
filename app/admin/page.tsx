...
import { BookOpen, Lock, User, ArrowRight, ShieldCheck, AlertCircle, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-[75vh] flex flex-col justify-center items-center px-4 py-8">
      <div className="w-full max-w-md bg-white border border-stone-200 shadow-xl rounded-3xl p-8 sm:p-10 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center text-center mb-8">
          ...
          <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-stone-100 border border-stone-200 text-stone-700 text-xs font-medium">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
            <span>Acesso Restrito do Autor</span>
          </div>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5 relative z-10">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 px-6 rounded-2xl bg-stone-900 text-white font-semibold text-sm shadow-md hover:bg-stone-800 active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
            {loading ? (
...
export default function AdminPage() {
  ...
  if (status === 'loading') {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-stone-500 gap-3">
        <Loader2 className="w-8 h-8 animate-spin text-stone-700" />
        <span className="text-xs font-mono">Verificando credenciais de administrador...</span>
      </div>
    );
  }
  ...
  return (
    <Suspense fallback={<div className="p-8 text-center text-stone-500">Carregando modo administração...</div>}>
      <HomeContent isAdmin={true} />
    </Suspense>
  );
}
